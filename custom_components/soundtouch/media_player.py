...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    def service_handle_snapshot_restore(call):
        """Handle the snapshot and restore of the playing media."""
        ...
        for entity_id in entity_ids:
            ...
            if selected_device is None:
                _LOGGER.warning(
                    "Unable to find SoundTouch device with entity_id: %s", str(entity_id)
                )
                return
...
class SoundTouchDevice(MediaPlayerEntity):
    ...
    def snapshot(self):
        """Snapshot media player."""
        self._device.snapshot()

        self._device._snapshotVolume = self.volume_level
